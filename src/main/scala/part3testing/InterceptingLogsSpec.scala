...
class InterceptingLogsSpec extends TestKit(ActorSystem("interceptingLogsSpecSystem", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  with WordSpecLike
  ...
  val item = "Rock the JVM Akka cource"
